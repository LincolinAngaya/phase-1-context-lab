...
function allWagesFor(recordArr) {
    ...
    const wagesOnDate = dateWorked.map(dateStamp => wagesEarnedOnDate(recordArr, dateStamp))
...
function calculatePayroll(recordArr) {
    ...
    recordArrOfArr.map(arr => {
        allWagesFor(arr)
        const wagesOnDate = arr.timeOutEvents.map(dateStamp => wagesEarnedOnDate(arr, dateStamp.date))
